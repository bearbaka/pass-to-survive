...
public abstract class PlayGameScreen implements Screen { // children of this class are only level
  ...
  public void update(float delta) {
    ...
    else if (!statsEnabled && buttons.getStats() != null && buttons.getStats().getStage() != null)
      buttons.getStats().remove();
    ...
    if (player.isEndingBouncer()) { // invoked when last two bouncers are touched by player
      ...
      cam.position.y = player.getBodyPositionY();
    ...
    cam.position.x = player.getBodyPositionX(); // camera moves with player
